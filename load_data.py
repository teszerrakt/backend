...
def merge_user_rating(user_rating, all_data):
    ...
    user_prefs = user_prefs.rename(columns={'id': 'comicID'}) # Renaming "id" with "comicID" to match with the .csv data
...
def load_rating_data(comic_genre, rating_data):
    ...
    cluster_dataset = pd.merge(cluster_dataset, comic_genre, on='comicID')
