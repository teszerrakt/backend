...
def find_neighbor(item_id, ratings, usage="prediction", metric='correlation', k=10, verbose=False):
    similarities = []
    indices = []
    ...
    if usage == "prediction":
        ...
        item_list = [ratings.index[indices.flatten()[i]] for i in range(0, len(indices.flatten()))]
        return item_list
...
def predict(user_id, item_id, ratings_cluster, centroids, metric='correlation', k=10, verbose=False):
    ...
    if verbose:
        print('Predicted rating for user {} -> item {}: {}'.format(user_id, item_id, prediction))
